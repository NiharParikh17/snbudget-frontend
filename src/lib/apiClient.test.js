...
describe('apiClient', () => {
  ...
  it('always sends credentials: "include"', async () => {
    ...
    await request('GET', '/api/users/abc')
  ...
  it('serializes the body and sets Content-Type for JSON requests', async () => {
    mockFetch(jsonResponse({ ok: true }))
    await request('POST', '/api/auth/login', { body: { identifier: 'x', password: 'y' } })
    const [, init] = globalThis.fetch.mock.calls[0]
  ...
  it('attaches the bearer token when provided', async () => {
    mockFetch(jsonResponse({ ok: true }))
    await request('POST', '/api/auth/logout', { accessToken: 'tok' })
    const [, init] = globalThis.fetch.mock.calls[0]
  ...
  it('throws ApiError with the server-provided message on non-2xx', async () => {
    ...
    await expect(request('POST', '/api/auth/login', { body: {} })).rejects.toMatchObject({
      name: 'ApiError',
      status: 401,
  ...
  it('wraps network failures in an ApiError with status 0', async () => {
    ...
    await expect(request('GET', '/api/users/abc')).rejects.toMatchObject({
      name: 'ApiError',
      status: 0,
    })
  })
